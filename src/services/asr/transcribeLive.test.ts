...
describe('transcribeWithAllProviders', () => {
  ...
  it('uses the only configured provider as primary', async () => {
    ...
    expect(result.attempts.openai.notConfigured).toBe(true);
  });

  it('prefers Sahara over the benchmark endpoints and OpenAI when all three succeed', async () => {
    process.env.SAHARA_API_KEY = 'fake-key';
    process.env.SAHARA_ASR_URL = 'https://infer.voice.intron.io/asr/v1/transcribe';
    process.env.MODEL_B_API_KEY = 'fake-key';
    process.env.MODEL_B_API_URL = 'https://example.com/model-b';
    process.env.OPEN_AI_KEY = 'fake-key';
