...
describe('provider capability & voice options', () => {
  ...
  it('never claims a provider supports a language it has no voice for at all, except gemini (fallback dropdown option)', () => {
    for (const [providerId, cap] of Object.entries(PROVIDER_CAPABILITIES)) {
      if (providerId === 'gemini') continue; // gemini voices intentionally act as a universal fallback
