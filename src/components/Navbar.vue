    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-15">
        ...
        <div class="hidden md:flex items-center gap-2.5">
          ...
          <div
            class="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs border"
            :class="providerStatus.openaiConfigured
              ? 'bg-blue-50 border-blue-200 text-blue-800'
              : 'bg-slate-50 border-slate-200 text-slate-600'"
          >
            <CheckCircle2 v-if="providerStatus.openaiConfigured" class="w-3.5 h-3.5 text-blue-700" />
            <AlertCircle v-else class="w-3.5 h-3.5 text-slate-400" />
            <span>OpenAI {{ providerStatus.openaiConfigured ? 'Ready' : 'Local' }}</span>
          </div>
