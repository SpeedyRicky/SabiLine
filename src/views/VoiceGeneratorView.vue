...
const selectedProvider = ref<ProviderId>('gemini');
const selectedVoiceId = ref('gemini-kore');
const speed = ref(1.0);
...
const voicesForDropdown = computed(() =>
  AVAILABLE_VOICES.filter((v) => v.language === selectedLang.value || v.provider === 'gemini')
);
