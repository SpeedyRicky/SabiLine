...
app.post("/api/intake/converse", asyncHandler(async (req, res) => {
  ...
  if (startCall === true) {
    const openingLanguage = typeof language === "string" && language !== "auto" && SUPPORTED_LANGUAGE_CODES.includes(language) ? language : null;
    ...
    const reply2 = await getSabiLineReply(Array.isArray(history) ? history : [], null, openingLanguage, 0);
    if (!reply2.success) {
    ...
    finalizeIntakeIfDone(resolvedVisitId, openingLanguage, reply2, priorLanguageHistory);
    return res.json({
      success: true,
      visitId: resolvedVisitId,
...
app.use((err, req, res, _next) => {
  ...
  const message = type === "entity.parse.failed" ? "The request body was not valid JSON." : type === "entity.too.large" ? "The request body was too large. Send a shorter audio clip." : status < 500 ? "The request could not be understood." : "Unexpected server error. Please try again.";
