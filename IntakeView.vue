...
const QUEUE_STORAGE_KEY_LANGUAGE = 'afrivoice_intake_language';

/** The languages the intake conversation actually supports. Kept local rather
 *  than imported from the ASR module, which is server-only (it reads
 *  process.env) and must never reach the client bundle. */
const INTAKE_LANGUAGES: LanguageCode[] = ['en', 'pcm', 'yo', 'ig', 'ha', 'ful'];

const OPENING_CAPTION =
  'Press SPEAK and just talk — SabiLine answers in whatever language you speak. No script, no language to pick.';

/** Caption shown while SabiLine listens without having greeted first — the
 *  case where the caller's language is not yet known, so there is no greeting
 *  to say. */
const LISTENING_CAPTION = 'Go ahead — say anything in your own language and SabiLine will answer in it.';
...
/** The language this browser last used with SabiLine. Persisted so a returning
 *  caller is GREETED in their own language rather than in English — the one
 *  case where SabiLine can open without having heard anything yet. It is only
 *  ever a starting hint: the language is still re-detected on every turn, so a
 *  wrong memory costs at most one sentence. */
const rememberedLanguage = ref<LanguageCode | null>(null);
...
onMounted(() => {
  ...
  // Storage can throw outright in private mode / with cookies blocked, and a
  // remembered language is a nicety — never let it break the call.
  try {
    const saved = localStorage.getItem(QUEUE_STORAGE_KEY_LANGUAGE);
    if (saved && INTAKE_LANGUAGES.includes(saved as LanguageCode)) {
      rememberedLanguage.value = saved as LanguageCode;
    }
  } catch {
    /* no remembered language */
  }
});
...
/** Records a freshly detected language, appending it to languageHistory only
 *  when it actually changed (so a run of turns in the same language doesn't
 *  produce a repeated entry) — used to surface a mid-call language switch to
 *  staff. See src/services/notify/staffNotify.ts. */
function recordLanguage(lang: LanguageCode | null | undefined) {
  if (!lang) return;
  detectedLanguage.value = lang;
  if (languageHistory.value[languageHistory.value.length - 1] !== lang) {
    languageHistory.value = [...languageHistory.value, lang];
  }
  // Remember it for next time, so the NEXT call opens in this language
  // instead of English.
  rememberedLanguage.value = lang;
  try {
    localStorage.setItem(QUEUE_STORAGE_KEY_LANGUAGE, lang);
  } catch {
    /* storage unavailable — the call still works, the next one just opens cold */
  }
}
...
async function handleSabiLineReply(data: IntakeConverseResponse & { visitId?: string }) {
  ...
  if (outcome.usedProvider === 'browser' && outcome.fallbackReason) {
    noticeHtml.value = 'OpenAI voice quota reached — spoke via device voice instead.';
  } else if (outcome.usedProvider === 'none' && outcome.fallbackReason) {
    noticeHtml.value = outcome.fallbackReason;
  }
  ...
  try {
    const res = await fetch('/api/intake/converse', {
      ...
      body: JSON.stringify({
        ...
        // Greet in the caller's own language when this browser already knows
        // it. 'auto' means "we have no idea yet" — and in that case SabiLine
        // does NOT open with an English greeting at all (see listenFirst
        // below); it listens and answers in whatever it hears.
        language: rememberedLanguage.value ?? 'auto',
      }),
    });
    ...
    if (!res.ok || !data.success) {
      ...
      noticeHtml.value = failureNotice(
        data,
        "OpenAI's quota is exhausted right now, so SabiLine can't start the call. Please try again later.",
    ...
    // No known language, so there is no greeting to say — go straight to
    // listening. SabiLine's first spoken sentence will be in the patient's
    // own language rather than an English opener nobody asked for.
    if (data.listenFirst) {
      phase.value = 'idle';
      caption.value = LISTENING_CAPTION;
      await startRecording();
      return;
    }

    // A greeting in a known language: record it BEFORE speaking so the voice
    // matches the text (speakAloud reads detectedLanguage).
    recordLanguage(data.detectedLanguage);
  ...
  try {
    const res = await fetch('/api/intake/converse', {
      ...
      body: JSON.stringify({
        ...
        elapsedMinutes,
        visitId: visitId.value,
        languageHistory: languageHistory.value,
      }),
    });
    const data: IntakeConverseResponse = await res.json();
    ...
    if (!res.ok || !data.success) {
      ...
      noticeHtml.value = failureNotice(
        data,
        "OpenAI's quota is exhausted right now, so SabiLine can't understand or respond at the moment. Please try again later.",
    ...
    if (!data.transcript) {
      ...
      noticeHtml.value = failureNotice(
        data,
        "OpenAI's quota is exhausted right now, so SabiLine can't understand speech at the moment. Please try again later.",
...
async function submitTyped() {
  ...
  // If the call hasn't started yet, this typed text IS the first turn — open
  // the call with it. It must not be discarded in favour of a spoken opening:
  // since SabiLine no longer greets in English when the language is unknown,
  // routing this through startCall() would open the microphone and silently
  // throw away what the patient just typed.
...
function finalizeVisit(reviewNeeded: boolean) {
  ...
  const record: IntakeRecord = {
    ...
    languageHistory: languageHistory.value.length ? languageHistory.value : [detectedLanguage.value ?? 'en'],
